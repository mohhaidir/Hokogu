...
export default function Detail() {
  ...
  useEffect(() => {
    ...
    axios({
      method: "get",
      url: `https://api.spoonacular.com/recipes/${id}/information?apiKey=${spoonKEY}`
    })
      .then(response => {
        ...
        {!loading && recipe !== null && (
          <div>
            {width > 1000 && (
              ...
              >
                <h1 className="accountText">{recipe.title}</h1>
              </div>
            )}
            {width <= 1000 && (
              <div
              ...
              >
                <h5 className="accountTextMini">{recipe.title}</h5>
              </div>
            )}
